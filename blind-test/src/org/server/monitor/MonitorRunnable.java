...
public final class MonitorRunnable  {
	...
	public final void start() {
		if(_thread.isAlive()) {
			...
			final String locFormat = String.format("Le thread " + _thread.getName() + " est déjà lancé. On ne peut le relancer.");
			final RuntimeException locException = new RuntimeException(locFormat);
			locFileProvider.appendMessage(Level.SEVERE, locFormat, locException);
			throw locException;
		}
		_thread.start();
	}
	...
	private final void showCommand(final String parCommandName) {
		final String locMessage = String.format("La commande \"%s\" est invalide. La liste des commandes disponible est : %s", parCommandName, EnumMonitorCommand.getCommands().toString());
		_monitor.println(locMessage);
	}
	
	private final void invoke(final String locCommandName, final String[] parArguments, final EnumMonitorCommand parEnumCommand) {
